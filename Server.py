...
COMMANDS = {
    ...
    'quit': ['Stops current connection with a client. To be used after client selection.'],
...
class MultiServer(object):
    ...
    def __init__(self):
        print('Welcome to the Verax reverse shell server. Enter the local port to use, Enter "help" for commands.')
        print('Your local ip address is: ' + socket.gethostbyname(socket.getfqdn()))
        now = datetime.datetime.now()
        print('The current date and time are' + ': ' + str(now.month) + '/' + str(now.day) + '/' + str(now.year)
              + ', ' + str(now.hour) + ':' + str(now.minute))
        self.host = ''
        self.port = int(input('Insert the local port to use.'))
        self.socket = None
